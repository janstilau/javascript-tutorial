...
语句以分号结尾，一个分号就表示一个语句结束。多个语句可以写在一行内。 (不要这样写)
...
作者在这里说, 建立引用关系. 和 JS 里面都是引用值有关系吗???
...
这是一个特殊的状态, 代表的没有无. 可以用 Swift 的 Optional 类型来考虑.
...
这里有问题, 和之前的语言习惯完全不相同. 所以, 应该废弃. 
之后应该不使用 var, let 的使用和之前语言使用习惯一致.
...
if 后面一定是一个 Bool 值. 如果不是 Bool 值, 会自动调用 Bool 的构造函数, 创建一个 Bool 值. 各个类型, 都有对于 Bool 的生成策略.
...
=== 严格相等, 并不是引用判等, 而是 类型判断之后, 再加 值相等判断. 
